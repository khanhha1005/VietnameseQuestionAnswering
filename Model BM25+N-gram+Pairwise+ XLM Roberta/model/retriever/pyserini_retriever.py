...
from model.utils.utils import init_logger
from model.reader.base import Context
...
def retriever(question, searcher, para_num=20):
    ...
    if type(searcher) == FaissSearcher:
        ...
        for r in results:
            ...
            hits.append((r.docid, hit, r.score))
    else:
        try:
            if language == "zh":
            ...
            return []
        hits = [(h.docid, h.raw, h.score) for h in hits]
    ...
    for i in range(0, len(hits)):
        ...
        for s in blacklist:
            if s in t:
                continue
        metadata = {}
        contexts.append(Context(t, id, language, metadata, score))
    return contexts
